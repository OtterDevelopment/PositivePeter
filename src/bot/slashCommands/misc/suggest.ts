...
export default class Ping extends SlashCommand {
	constructor(client: BetterClient) {
		super("suggest", client, {
			description: `Suggest a trigger to be added into Positive Peter.`,
			options: [
				{
					name: "trigger",
	...
	override async run(interaction: CommandInteraction) {
		const blocked = await this.client.mongo
			.db("users")
			.collection("blocked")
			.findOne({ userId: this.client.functions.hash(interaction.user.id) });
		if (blocked)
		...
		await this.client.logger.webhookLog("suggestion", {
			...
			embeds: [
				new MessageEmbed({
					...
					footer: {
						text: `Hashed User ID: ${this.client.functions.hash(interaction.user.id)}`
					},
					timestamp: Date.now(),
