...
export default class TextCommand {
	...
	public async validate(
		...
		if (this.guildOnly && !message.inGuild())
			...
			return {
				title: "Missing Permissions",
				description: "This command can only be used by the bot developer."
			};
			...
			return {
				...
				description: `You need ${
					...
					.map(
						(permission) => `**${this.client.functions.getPermissionName(permission)}**`
					)
					.join(", ")} permission${
				...
				if (Date.now() - onCooldown[message.author.id] < this.cooldown)
					return {
						...
						description: `This command is still on cooldown for another ${format(
							onCooldown[message.author.id] + this.cooldown - Date.now(),
							true
						)}!`
					};
		}
