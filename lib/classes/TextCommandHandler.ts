...
export default class TextCommandHandler {
	...
	public async handleCommand(message: BetterMessage) {
		...
		if (missingPermissions)
			return message.reply(this.client.functions.generateErrorMessage(missingPermissions));

		return this.runCommand(command, message, args);
	}

	private async runCommand(command: TextCommand, message: BetterMessage, args: string[]) {
